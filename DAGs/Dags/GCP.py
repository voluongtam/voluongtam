...
from airflow.providers.google.cloud.operators.biggquery import (
...
from airflow.providers.googl.cloud.transfers.gcs_to_bigquery import (
...
dag = DAG(
    ...
    end_date=datetime.datetime(year=2021,month=11,day=1),
    schedule_interval="@daily",
    default_args={"dependes_on_past": True},
)
...
def _fetch_ratings(api_conn_id, gcp_conn_id, gcs_bucket, **context):
    ...
    api_hook = MovelensHook(conn_id=api_conn_id)
    ratings = pd.DataFrame.from_records(api_hook.get_ratings_for_month(year=year, month=month),
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_path = path.join(tmp_dir, "rating.csv")
...
query_top_ratings = BigQueryExecuteQueryOperator(
    ...
    sql=(
        ...
        "FROM " + os.environ["BIGQUERY_DATESET"] + ".ratings "
